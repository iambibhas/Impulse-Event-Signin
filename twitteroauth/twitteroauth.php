...
class TwitterOAuth {
  ...
  /**
  * Save the ID of last got mention..
  */
  ...
  function sendError($user,$status_id){
      $curlUrl="http://localhost/indialarm/tweet.php";
      echo $curlUrl;
    $ch = curl_init($curlUrl);
    
    if(curl_errno($ch) == 0 || curl_errno($ch) == ""){
        $args="";
        $args = "msg=@{$user} Invalid Request format. [refer: http://twitter.com/{$user}/status/{$status_id} ]";
  ...
  function secondsToWords($seconds)
    ...
    /*** return the string ***/
            return sprintf("%d $plural, %d hours, %d min", $days, $hours, $mins);
    }
}
